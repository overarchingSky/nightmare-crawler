...
export default {
    ...
    watch:{
        'task.enable'(val){
            if(val){
                //开启任务
                console.log('开启任务',this.task.taskName)
  ...
  methods:{
    getProducts(){
        console.log('获取任务关联的商品信息')
        return new Promise((resolve,reject) => {
            //获取发布的商品明细列表
            if(!this.products || !Array.isArray(this.products)){
                console.log('发送商品信息事件',this.task.id,ipcRenderer)
    ...
    releaseProduct(){        
        ...
        store.event.dispatch('release-product','back',this.products)
