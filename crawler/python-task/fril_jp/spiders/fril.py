...
class FrilSpider(scrapy.Spider):
    ...
    def parseProd(self, response):
        ...
        for prodSelector in prods:
            detailUrl = 'https://fril.jp' + prodSelector.xpath(".//a/@href").getall()[2].strip().replace('\\"','')
            print('detailUrl')
            print(detailUrl)
    ...
    def parseDetail(self, response):
        print('parseDetail:')
        print(response)
        detail = response.xpath('.//form[id=item-form')
        #detail = response.xpath('.//form[id="item-form"]/div[data-rect-class="ItemContent"]/@data-react-props').get()
        print(detail)
        yield detail
